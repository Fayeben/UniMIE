...
class GaussianDiffusion_anysize_v1:
    """
    ...
    def condition_score(self, cond_fn, p_mean_var, x, pred_xstart, t, light_factor=None, light_mask=None, corner=None, model_kwargs=None):
        ...
        light_factor, light_mask, gradient = cond_fn(x, self._scale_timesteps(t), light_factor=light_factor, light_mask=light_mask, corner=corner, **model_kwargs)
            ...
            if cond_fn is not None:
                for i in range(2):
                    out, light_factor_tmp, light_mask_tmp = self.condition_score(
                        cond_fn, out, x[:, :, hi:hi + p_size, wi:wi + p_size], out["pred_xstart"], t, light_factor = light_factor, light_mask = light_mask[hi:hi + p_size, wi:wi + p_size], corner=[hi, wi, p_size], model_kwargs=model_kwargs
                    )
