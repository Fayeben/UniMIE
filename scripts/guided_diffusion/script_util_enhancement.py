...
from . import gaussian_diffusion_enhancement_anysize as gd_anysize
from . import gaussian_diffusion_enhancement_HDR as gd_HDR
from .respace_enhancement import SpacedDiffusion_anysize, SpacedDiffusion_HDR, space_timesteps
...
def create_model_and_diffusion_anysize(
    image_size,
    class_cond,
    learn_sigma,
    num_channels,
    ...
    diffusion = create_gaussian_diffusion_anysize(
        steps=diffusion_steps,
        learn_sigma=learn_sigma,
        noise_schedule=noise_schedule,
        use_kl=use_kl,
...
def create_gaussian_diffusion_anysize(
    *,
    steps=1000,
    learn_sigma=False,
    sigma_small=False,
    ...
    betas = gd_anysize.get_named_beta_schedule(noise_schedule, steps)
    # betas is a 1D numpy array of length steps
    if use_kl:
        loss_type = gd_anysize.LossType.RESCALED_KL
    elif rescale_learned_sigmas:
        loss_type = gd_anysize.LossType.RESCALED_MSE
    else: # default type
        loss_type = gd_anysize.LossType.MSE
    if not timestep_respacing:
        timestep_respacing = [steps]
    ...
    # it contains timestep_respacing integers that uniformly covers the interval
    # learn_sigma is true in our case, rescale_timesteps is false in our time
    return SpacedDiffusion_anysize(
        use_timesteps=space_timesteps(steps, timestep_respacing),
        betas=betas,
        model_mean_type=(
            gd_anysize.ModelMeanType.EPSILON if not predict_xstart else gd_anysize.ModelMeanType.START_X
            # gd.ModelMeanType.PREVIOUS_X if not predict_xstart else gd.ModelMeanType.EPSILON
        ),
        model_var_type=(
            (
                gd_anysize.ModelVarType.FIXED_LARGE
                if not sigma_small
                else gd_anysize.ModelVarType.FIXED_SMALL
            )
            if not learn_sigma
            else gd_anysize.ModelVarType.LEARNED_RANGE
        ),
        loss_type=loss_type,
        rescale_timesteps=rescale_timesteps,
    )
