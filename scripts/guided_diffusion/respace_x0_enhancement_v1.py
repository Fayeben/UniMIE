...
from .gaussian_diffusion_x0_enhancement_anysize_v1 import GaussianDiffusion_anysize_v1
from .gaussian_diffusion_x0_enhancement_HDR_v1 import GaussianDiffusion_HDR_v1
...
class SpacedDiffusion_anysize_v1(GaussianDiffusion_anysize_v1):
    """
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = GaussianDiffusion_anysize_v1(**kwargs)  # pylint: disable=missing-kwoa
...
class SpacedDiffusion_HDR_v1(GaussianDiffusion_HDR_v1):
    """
    A diffusion process which can skip steps in a base diffusion process.
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = GaussianDiffusion_HDR_v1(**kwargs)  # pylint: disable=missing-kwoa
        # self.base_diffusion = base_diffusion
        # base_diffusion.betas is of length original_num_steps
        last_alpha_cumprod = 1.0
        new_betas = []
...
class _WrappedModel1:
    ...
    def __call__(self, x, ts, light_factor, light_variance, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(x, new_ts, light_factor, light_variance, **kwargs)
