...
from .gaussian_diffusion_enhancement_anysize_v1 import GaussianDiffusion_anysize_v1
from .gaussian_diffusion_enhancement_HDR_v1 import GaussianDiffusion_HDR_v1
...
class _WrappedModel1:
    ...
    def __call__(self, x, ts, light_factor, light_mask, corner, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
